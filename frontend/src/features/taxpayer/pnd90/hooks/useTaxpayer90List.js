...
import { Taxpayer90Service } from '../service/taxpayer90Service';
import { UnifiedService } from '../../../main/services/unifiedService';
import { useTaxpayerFilters } from '../../../main/hooks/useTaxpayerFilters';

/**
 * Hook สำหรับจัดการข้อมูลรายการ ภ.ง.ด. 90
 * ครอบคลุมการดึงข้อมูล, การค้นหา, การกรองความเสี่ยง, และการประมวลผลข้อมูลอัตโนมัติเบื้องหลัง
 */
export const useTaxpayer90List = (storageKey, initialParams = {}) => {
    const {
        filters,
        setPage,
        setSearchQuery,
    ...
    // ตรวจสอบว่าเป็นการแบ่งหน้าจาก Server หรือจาก Client (หากมีการค้นหาคีย์เวิร์ดจะใช้ Client)
    const isServerPaged = !searchQuery;
    const searchTimeoutRef = useRef(null);
    const fetchIdRef = useRef(0);

    // ฟังก์ชันสำหรับเรียกข้อมูลจาก API (ดึงรายการปกติ หรือ ค้นหาตามคำสำคัญ)
    const fetchData = useCallback(async (query = '') => {
        ...
        try {
            let result;
            if (query) {
                result = await Taxpayer90Service.searchByKeyword(query, false, year, officeParams);
                setApiMeta({ totalPages: 0, totalElements: 0 });
            } else {
                result = await Taxpayer90Service.getAll(
            ...
            console.error(`Fetch PND 90 Error:`, err);
    ...
    // ประมวลผลข้อมูล (ค้นหา, กรองตามความเสี่ยง, และจัดเรียงลำดับ)
    const processedData = useMemo(() => {
        ...
        if (filterRisk !== 'all') {
            filtered = filtered.filter(item => {
                const hasRisks = item.isEnriched && Object.values(item.risks || {}).some(Boolean);
                if (filterRisk === 'at_risk') return hasRisks;
                if (filterRisk === 'normal') return !hasRisks;
                if (!item.isEnriched) return true;

                if (filterRisk === 'income_less') return item.risks?.incomeDecrease;
                if (filterRisk === 'tax_less') return item.risks?.taxAmtDecrease;
                if (filterRisk === 'no_sent_90') return item.risks?.sentBaseNotCompare;
                if (filterRisk === 'spouse_income') return item.risks?.spouseHasIncome;
                return true;
            });
        }
        ...
        if (quickFilter && quickFilter !== 'all') {
            filtered = filtered.filter(item => {
                if (quickFilter === 'highIncome') return (item.totalIncome || 0) > 1000000;
                if (quickFilter === 'overdue') return item.isEnriched && item.risks?.sentBaseNotCompare;
                return true;
            });
        }
        ...
        return filtered;
    }, [allData, searchQuery, sortBy, filterRisk, quickFilter, isServerPaged]);
    ...
    // ฟังก์ชันดึงสถานะความเสี่ยงเพิ่มเติมเป็นรายบุคคล (Manual Enrichment)
    const handleEnrich = async (id, dln) => {
        ...
        try {
            ...
            console.error("PND 90 Single Enrichment Error:", err);
            setAllData(prevData => prevData.map(d =>
