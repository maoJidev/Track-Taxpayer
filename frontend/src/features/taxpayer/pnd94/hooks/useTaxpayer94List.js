...
export const useTaxpayer94List = (storageKey, initialParams = {}) => {
    ...
    const fetchData = useCallback(async (query = '') => {
        ...
        try {
            let result;
            if (query) {
                result = await Taxpayer94Service.searchPND94(query, year, officeParams);
                setApiMeta({ totalPages: 0, totalElements: 0 });
            } else {
                result = await Taxpayer94Service.getAll(
            ...
            console.error(`Fetch PND 94 Error:`, err);
    ...
    const processedData = useMemo(() => {
        ...
        if (filterRisk !== 'all') {
            filtered = filtered.filter(item => {
                ...
                if (filterRisk === 'has_pnd94') return item.risks?.hasPND94;
                return true;
            });
        }
        ...
        if (quickFilter && quickFilter !== 'all') {
            filtered = filtered.filter(item => {
                ...
                if (quickFilter === 'hasPnd94') return item.risks?.hasPND94;
                return true;
            });
        }
    ...
    const setSortBy = (val) => updateFilter('sortBy', val);
    const setRiskFilter = (val) => updateFilter('filterRisk', val);
    ...
    const handleEnrich = async (id, dln) => {
        ...
        try {
            ...
            console.error("PND 94 Single Enrichment Error:", err);
            setAllData(prevData => prevData.map(d =>
    ...
    // Background enrichment — processes visible items first
