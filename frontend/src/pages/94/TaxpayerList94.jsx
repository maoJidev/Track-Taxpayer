...
const TaxpayerList94 = () => {
    ...
    const {
        ...
        handleRefresh,
        handleEnrich
    } = useTaxpayer94List('taxpayer_list_94_state', user?.offices);
    ...
    const handleOfficeChange = (offices) => {
        console.log('[TaxpayerList94] Office selection changed:', offices);
        setOffices(offices);
    };
    ...
    const handleRowClick = (item) => {
        ...
        const dln = item.dln || '-';
        if (taxId) {
            navigate(`/taxpayers/pnd94/detail/${encodeParam(taxId)}/${encodeParam(dln)}/${year || '67'}`);
        }
    };
    ...
    return (
        <div className="container-fluid bg-light min-vh-100 py-4 px-4 font-sarabun">
            ...
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h3 className="text-navy fw-bold mb-1">รายชื่อผู้เสียภาษี ภ.ง.ด. 94</h3>
                    <p className="text-secondary mb-0 small">ระบบตรวจสอบรายได้และความเสี่ยงอัตโนมัติ (ภ.ง.ด. 94)</p>
                </div>
                                ...
                                <tr>
                                    <td colSpan="8" className="text-center py-5">
                                        <div className="py-4">
                                            ...
                                            <p className="text-muted small mb-0">โปรดเลือก <b>ภาค</b> หรือ <b>พื้นที่</b> เพื่อเริ่มต้นแสดงข้อมูลผู้เสียภาษี (ภ.ง.ด. 94)</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : data.length === 0 ? (
                                    ...
                                    <tr key={`${item.taxId}-${item.dln}`} onClick={() => handleRowClick(item)} style={{ cursor: 'pointer' }}>
                                        ...
                                        <td className="px-4 text-end">
                                            <div className="d-flex flex-column align-items-end">
                                                <div className="text-secondary mb-1" style={{ fontSize: '11px' }}>ภ.ง.ด. 94</div>
